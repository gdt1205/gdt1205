...
def lichengji_calculate(odom_data_list):
    ...
    offset_x_cm = 0 #odom_dict.get('x')
    offset_y_cm = 0 #odom_dict.get('y')
    absolute_yaw = odom_dict.get('yaw')

    # 确保所有需要的值都成功获取
    if offset_x_cm is None or offset_y_cm is None or absolute_yaw is None:
    ...
    # 应用旋转矩阵
    offset_x_world_cm = offset_x_cm * math.cos(robot_yaw_world_rad) - offset_y_cm * math.sin(robot_yaw_world_rad)
    offset_y_world_cm = offset_x_cm * math.sin(robot_yaw_world_rad) + offset_y_cm * math.cos(robot_yaw_world_rad)

    # 4. 计算在世界坐标系下的最终坐标
    now_x = START_X + offset_x_world_cm
    now_y = START_Y + offset_y_world_cm
...
def leida_calculate(lidar_data, odom_data):
    ...
    for angle_deg, distance in lidar_data:
        ...
        wx_hit = now_car_x + (distance/35) * math.cos(absolute_angle)
        wy_hit = now_car_y + (distance/35) * math.sin(absolute_angle)
...
def plot_data(ax, all_robot_poses, all_obstacle_points, save_path: Optional[str]=None):
    """
    绘制小车与障碍物点。
    ...
    if not all_robot_poses:
        print("plot_data: 缺少里程计位姿，无法绘图。")
        return

    lichengji_data = all_robot_poses[-1]
    ...
    zhangai_data = all_obstacle_points
    # 绘制障碍物点
    if zhangai_data:
    ...
    path_x = [p[0] for p in all_robot_poses]
    path_y = [p[1] for p in all_robot_poses]
    ...
    head_angle_rad = np.deg2rad(now_theta)  # 同上转换
...
if __name__ == "__main__":
    ...
    dongzuo = ('1', '3', '1', '2', '4')#动作序列，1表示前进，2表示左转，3表示右转，4表示掉头
        ...
        try:
            ...
            for action in dongzuo:
                ...
                if lichengji_data and zhangai_data:
                    ...
                    all_robot_poses.append(lichengji_data)
                    all_obstacle_points.extend(zhangai_data)
                    plot_data(ax, all_robot_poses, all_obstacle_points)
                    plt.pause(0.01)
